...
function AllUsers() {
  ...
  useEffect(() => {
    if (firstLoading) {
      ...
      const url = `/admin/users?page=${currentPage}&limit=${currentLimit}&filter=${currentFilter}&sort=${sortBy}&search=${search}`;
      fetchData(url);
    }
  ...
  useEffect(() => {
    if (search.length >= 2) {
      ...
      const sortBy = `${currentSort[0]},${currentSort[1] ? "-1" : "1"}`;
      const url = `/admin/users?page=${currentPage}&limit=${currentLimit}&filter=${currentFilter}&sort=${sortBy}`;
      fetchData(url);
    }
  }, [search]);
  ...
  return (
    <div>
      <h1 className="my-2 text-lg ml-2">All users</h1>
